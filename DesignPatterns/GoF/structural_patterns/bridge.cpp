// Source: http://cpp-reference.ru/patterns/structural-patterns/bridge/

/*
    Паттерн Bridge разделяет абстракцию и реализацию на две отдельные иерархии классов
    так, что их можно изменять независимо друг от друга.

    Первая иерархия определяет интерфейс абстракции, доступный пользователю.
    Все детали реализации, связанные с особенностями среды скрываются во второй иерархии.

    Достоинства паттерна Bridge

    - Проще расширять систему новыми типами за счет сокращения общего числа родственных
    подклассов.
    ...
    реализации пользовательский код не требует перекомпиляции.
*/
...
int main()
{
    Logger * p = new FileLogger( std::string("log.txt"));
